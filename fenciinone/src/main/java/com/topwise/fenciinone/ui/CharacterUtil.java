...
class CharacterUtil {
	...
	public static int identifyCharType(char input){
		Log.d("guohao-3","[identifyCharType] ");
		if(input >= '0' && input <= '9'){
			//阿拉伯数字
			Log.d("guohao-3","char = "+input+",type = CHAR_ARABIC");
			return CHAR_ARABIC;
			
		}else if((input >= 'a' && input <= 'z')
				|| (input >= 'A' && input <= 'Z')){
			//英文字母
			Log.d("guohao-3","char = "+input+",type = CHAR_ENGLISH");
			return CHAR_ENGLISH;
			...
			if(ub == Character.UnicodeBlock.CJK_UNIFIED_IDEOGRAPHS
				...
				//目前已知的中文字符UTF-8集合
				Log.d("guohao-3","char = "+input+",type = CHAR_CHINESE");
				return CHAR_CHINESE;
				...
				Log.d("guohao-3","char = "+input+",type = CHAR_OTHER_CJK");
		...
		//少了对标点符号的判断
		//其他的不做处理的字符
		Log.d("guohao-3","char = "+input+",type = CHAR_USELESS");
		return CHAR_USELESS;
		// TODO 这样写就可以显示标点符号
		//return CHAR_ENGLISH;
	}
