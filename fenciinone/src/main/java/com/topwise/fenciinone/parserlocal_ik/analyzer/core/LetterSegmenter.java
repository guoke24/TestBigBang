...
class LetterSegmenter implements ISegmenter {
	...
	public void analyze(AnalyzeContext context) {
		Log.d("guohao-2","[LetterSegmenter.analyze] 开始");
		...
		Log.d("guohao-2","[LetterSegmenter.analyze] 结束");
	}
...
//			}
				...
				//遇到非Letter字符，输出词元
				Log.d("guohao-410","输出词元"
						+",当前curosr = "+context.getCursor()
						+",偏移量 = "+context.getBufferOffset()
						+",词元开始位置 = "+this.start
						+",词元结束位置 = "+this.end
						+",词元长度 = "+ (this.end - this.start + 1)
						+",词元类型 = " + Lexeme.TYPE_LETTER
				);
	...
	private boolean processEnglishLetter(AnalyzeContext context){
		Log.d("guohao-2","[LetterSegmenter.processEnglishLetter] 开始");
		Log.d("guohao-2"," 当前指针位置 = "+context.getCursor());
		...
		if(this.englishStart == -1){//当前的分词器尚未开始处理英文字符
			...
			if(CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()){
				...
				Log.d("guohao-2"," 记录当前指针位置为结束位置 englishEnd = "+englishEnd);
				Log.d("guohao-410",""
					+""
				);
			}else{
				//遇到非English字符,输出词元
				Log.d("guohao-410","识别为结束标志，输出词元"
						+",当前curosr = "+context.getCursor()
						+",当前字符 = "+context.getCurrentChar()
						+",偏移量 = "+context.getBufferOffset()
						+",词元开始位置 = "+this.englishStart
						+",词元结束位置 = "+this.englishEnd
						+",词元长度 = "+ (this.englishEnd - this.englishStart + 1)
						+",词元类型 = " + Lexeme.TYPE_LETTER
				);
		...
		if(context.isBufferConsumed()){
			...
			if(this.englishStart != -1 && this.englishEnd != -1){
				...
				this.englishStart = -1;
				this.englishEnd= -1;
				Log.d("guohao-2"," 缓冲以读完，输出词元 = "+newLexeme.toString());
			}
		}
		...
		Log.d("guohao-2","[LetterSegmenter.processEnglishLetter] 结束，返回 对缓冲区解锁 = "+needLock);
		return needLock;			
	}
